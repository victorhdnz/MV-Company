...
-- Apenas admins podem inserir/atualizar (via API)
...
CREATE TABLE IF NOT EXISTS plan_features (
  ...
  monthly_limit INTEGER, -- NULL = ilimitado
...
CREATE TABLE IF NOT EXISTS ai_agents (
  ...
  is_premium BOOLEAN DEFAULT FALSE, -- Só disponível para Pro
...
-- 7. SUPPORT_TICKETS - Tickets de suporte (para envio de acessos)
-- =====================================================
...
CREATE TABLE IF NOT EXISTS support_messages (
  ...
  is_internal BOOLEAN DEFAULT FALSE, -- Notas internas da equipe
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
INSERT INTO courses (slug, title, description, thumbnail_url, instructor_name, duration_hours, plan_required, is_published, order_position) VALUES
('canva-basico', 'Canva do Zero ao Avançado', 'Aprenda a criar designs profissionais no Canva, desde o básico até técnicas avançadas.', '/courses/canva.jpg', 'Gogh Lab', 8.5, 'all', true, 1),
('capcut-edicao', 'Edição de Vídeo com CapCut', 'Domine a edição de vídeos para redes sociais usando o CapCut Pro.', '/courses/capcut.jpg', 'Gogh Lab', 6.0, 'all', true, 2),
('reels-virais', 'Criação de Reels Virais', 'Aprenda técnicas e tendências para criar Reels que viralizam.', '/courses/reels.jpg', 'Gogh Lab', 4.0, 'gogh_pro', true, 3),
('ia-conteudo', 'IA para Criadores de Conteúdo', 'Use inteligência artificial para turbinar sua produção de conteúdo.', '/courses/ia.jpg', 'Gogh Lab', 5.0, 'gogh_pro', true, 4)
ON CONFLICT (slug) DO NOTHING;
...
-- 11. COURSE_LESSONS - Aulas dos cursos
-- =====================================================
...
-- 12. USER_COURSE_PROGRESS - Progresso nos cursos
-- =====================================================
...
-- 13. USER_NICHE_PROFILE - Perfil de nicho do usuário (para treinar agentes)
-- =====================================================
CREATE TABLE IF NOT EXISTS user_niche_profiles (
  ...
  content_pillars JSONB DEFAULT '[]'::jsonb, -- pilares de conteúdo
  competitors JSONB DEFAULT '[]'::jsonb, -- concorrentes
  goals TEXT, -- objetivos
  platforms JSONB DEFAULT '[]'::jsonb, -- plataformas que usa
...
-- 14. TOOL_ACCESS_CREDENTIALS - Credenciais de acesso às ferramentas
-- (Armazena de forma segura os acessos enviados ao usuário)
-- =====================================================
...
BEGIN
  ...
  -- Verificar se o recurso está habilitado para o plano
...
-- Função para incrementar uso de recurso
...
-- Função para obter uso atual do usuário
...
DECLARE
  t TEXT;
BEGIN
  FOREACH t IN ARRAY ARRAY['subscriptions', 'ai_conversations', 'support_tickets', 'courses', 'user_niche_profiles', 'tool_access_credentials']
  LOOP
    EXECUTE format('
...
-- =====================================================
COMMENT ON TABLE subscriptions IS 'Assinaturas dos usuários sincronizadas com Stripe';
COMMENT ON TABLE plan_features IS 'Recursos disponíveis por plano (Essencial/Pro)';
COMMENT ON TABLE user_usage IS 'Uso mensal de recursos por usuário';
COMMENT ON TABLE ai_agents IS 'Agentes de IA disponíveis no sistema';
COMMENT ON TABLE ai_conversations IS 'Conversas dos usuários com os agentes';
COMMENT ON TABLE ai_messages IS 'Mensagens individuais das conversas';
COMMENT ON TABLE support_tickets IS 'Tickets de suporte e solicitação de acessos';
COMMENT ON TABLE courses IS 'Cursos disponíveis na plataforma';
COMMENT ON TABLE course_lessons IS 'Aulas dos cursos';
COMMENT ON TABLE user_course_progress IS 'Progresso do usuário nos cursos';
COMMENT ON TABLE user_niche_profiles IS 'Perfil de nicho do usuário para personalização dos agentes';
COMMENT ON TABLE tool_access_credentials IS 'Credenciais de acesso às ferramentas (Canva/CapCut)';
